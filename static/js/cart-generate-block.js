...
function ShowCart() {
	...
	if (Bly == null) {
		...
		for (var i = 0; i < Bly.length; i++) {
			div += '<div class="container-eat-cart"><div class="container-image-eat-cart"><img src="' + Bly[i].Image + '"><div class="container-name-eat-cart"><span>' + Bly[i].Name + '</span></div></div><div class="container-button-eat-cart"><div class="container-price-eat-cart"><span>' + Bly[i].Price + ' руб.</span></div><div class="card-dish-action"><div data-id-dish="' + Bly[i].id + '" class="inpMinus">-</div><input class="dishCount" type="number" step="1" min="1" max="50" name="dishCount" value="' + Bly[i].CountDish + '" readonly><div data-id-dish="' + Bly[i].id + '" class="inpPlus">+</div><div data-id-dish="' + Bly[i].id + '" class="container-button-cancel-eat-cart"><span class="fa fa-times"></span></div></div></div></div>';
		}
		BlockContainer.innerHTML = div;
		TotalCart();
	}
