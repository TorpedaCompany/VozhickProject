	...
	function cart() {
		...
		if (Arr != null) {
			...
			for (let i = 0; i < length; i++) {
				if (Arr[i].id == IdDish) {
					if (PriceDishBlock[0].getAttribute("data-portions-status") == "true") {
						console.log('Значение в объекте' + Arr[i].CountDish);
						CountDishArr = Arr[i].CountDish;
						CountDishArr += CountDish;
						Arr[i].CountDish = CountDishArr;

						console.log('Значение в инпуте' + CountDish);
						console.log('Конечное значение' + CountDishArr);
